...
## Обновление до 1.0.1: первый вход и восстановление
...
Версия 1.0.1 установлена и проверена на физическом Raspberry Pi с Debian 13. Для Heltec V4 создан полный резервный образ, плата прошита официальной USB Companion 1.17.1 и подключена по USB serial. Радиообмен и региональные настройки ещё требуют отдельной проверки.
...
Установщик использует зафиксированные в релизе версии Python-зависимостей. Новое виртуальное окружение собирается отдельно и заменяет рабочее только после успешной проверки; при ошибке запуска сохраняется предыдущая версия. Для загрузки зависимостей при установке пока требуется доступ к PyPI.
...
Установка из исходников использует тот же пакетный установщик: выполните `sudo bash scripts/install.sh` из репозитория. Предыдущее окружение сохраняется в `/opt/meshcore-pi-station/` для ручного восстановления. При неудачном запуске возвращается прежнее окружение; метаданные Debian-пакета и изменения базы данных автоматически не откатываются.
...
Служебный пользователь `meshcore` автоматически добавляется в `dialout`, а также в `plugdev` для native USB ESP32-S3 и `video` для диагностики питания Raspberry Pi, если эти группы существуют.
